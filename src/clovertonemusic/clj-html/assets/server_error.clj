...
(def html-assets-server_error
 [:html
  {:lang "en-us"}
  [:head
   [:title "Server Error - Clovertone Music"]
   [:meta
  ...
  [:body
   [:div#page
    ...
    [:div#main.right
     [:div#contents
      [:div#content
       [:h1.title "Server Error"]
       [:p "Sorry, there was an error while processing your request."]
       [:p [:a {:href "/"} "Continue browsing Clovertone Music."]]]]
     [:div#charts]
     [:div#users]]
    [:div#audio-box]
...
(defn show-html-assets-server_error [request]
  {:status 200
   :headers {"Content-Type" "text/html"}
   :body (page/html html-assets-server_error)})
